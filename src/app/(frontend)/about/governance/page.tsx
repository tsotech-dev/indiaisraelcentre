...
export default async function GovernancePage() {
  const c = await getGlobal('about-governance')
    ...
    <>
        ...
        <div className="max-w-3xl mx-auto px-6 py-14 space-y-6 text-stone-700 leading-relaxed text-lg">
          {(['paragraph1', 'paragraph2', 'paragraph3'] as const).map((key, i) => (
            <Reveal key={key} delay={i * 100}>
              <p>{(c?.[key] as string | undefined) ?? DEFAULTS[key]}</p>
            </Reveal>
          ))}
        </div>
      </section>
      <section className="bg-iic-paper border-b border-stone-200">
        <div className="max-w-5xl mx-auto px-6 py-16">
          <Reveal>
            <div className="text-[11px] font-sans font-bold uppercase tracking-[0.24em] text-iic-saffron mb-8">/ Board of Directors</div>
          </Reveal>
            ...
            {BOARD.map((m, i) => (
              <Reveal key={i} delay={Math.min(i * 80, 240)}>
                <div className="bg-white p-6 h-full">
                  ...
                  <div className="text-[10px] font-sans font-bold uppercase tracking-[0.18em] text-iic-saffron-deep mb-1">{m.title}</div>
                  <div className="text-xs text-stone-500">{m.affiliation}</div>
                </div>
              </Reveal>
            ))}
          </div>
          <p className="text-xs font-sans text-stone-400 mt-6 italic">Board composition to be populated prior to launch.</p>
        </div>
      </section>
    </>
  )
