...
const DEFAULT_AREAS = [
  { label: 'Academic Dialogue.', body: 'Sustained intellectual exchange between scholars of India and Israel.' },
  { label: 'Policy Conversations.', body: 'Structured engagement between policy professionals, practitioners and researchers.' },
  {
  ...
  { label: 'Research and Analysis.', body: 'Long papers, policy briefs and commentary across the six thematic pillars.', href: '/research/' },
  { label: 'Events and Roundtables.', body: "The Forum's convening programme.", href: '/forum/' },
]
...
export default async function AboutPage() {
  ...
  const eyebrow = (c?.eyebrow as string | undefined) ?? 'An Independent Policy Research Forum'
  const title = (c?.title as string | undefined) ?? 'India Israel Centre'
  const subtitle =
    (c?.subtitle as string | undefined) ?? 'Advancing Scholarship and Policy on India-Israel Relations'
  const openingFraming = (c?.openingFraming as string | undefined) ?? DEFAULT_OPENING_FRAMING

  const areasList = (areas?.areas as { label: string; body: string; href?: string }[] | null)?.length
    ? (areas?.areas as { label: string; body: string; href?: string }[])
    : DEFAULT_AREAS

  const stratText = (strat?.paragraph1 as string | undefined) ?? DEFAULT_STRATEGIC_OBJECTIVE

  const aboutBody =
    (c?.aboutBodyParagraphs as { body: string }[] | null)?.length
      ? (c?.aboutBodyParagraphs as { body: string }[]).map((p) => p.body)
      : DEFAULT_ABOUT_BODY

  const chairSummary = (c?.chairSummary as string | undefined) ?? DEFAULT_CHAIR_SUMMARY
  const partnersIntro = (partners?.paragraph1 as string | undefined) ?? DEFAULT_PARTNERS
  ...
  return (
    <>
      ...
      <section className="relative bg-mesh border-b border-stone-200 overflow-hidden">
        ...
        />
        <div className="relative max-w-7xl mx-auto px-6 pt-20 pb-0 overflow-hidden">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-end">
            <div className="lg:col-span-7 pb-20">
              ...
              <p className="text-lg text-stone-600 leading-relaxed max-w-xl animate-fade-up delay-3">
                {openingFraming}
              </p>
            </div>
            <div className="hidden lg:block lg:col-span-5 self-end">
              <ImagePlaceholder
                alt="India Israel Centre exterior or seminar hall — architectural photograph, clean natural light, 4:5 portrait orientation"
                aspectRatio="4/5"
        ...
        <div className="max-w-5xl mx-auto px-6 py-20">
          <Reveal>
            ...
            <p className="text-stone-500 mb-10">
              The Centre&rsquo;s activity is organised across five areas:
            </p>
          </Reveal>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {areasList.map((a, i) => (
              <Reveal key={i} delay={Math.min(i * 60, 300)}>
                <div className="bg-iic-paper border border-stone-200 rounded-lg p-6 hover-lift hover:border-iic-saffron/30 transition-colors">
          ...
          <Reveal delay={360}>
            <div className="mt-8">
              <Link href="/about/areas-of-engagement/" className="text-sm font-sans font-semibold text-iic-navy underline-anim">
                Read more →
              </Link>
            </div>
          </Reveal>
      ...
      <section className="bg-iic-navy text-white border-b border-white/10 overflow-hidden">
        <div className="relative max-w-5xl mx-auto px-6 py-20">
          ...
          <Reveal>
            ...
            <p className="text-xl text-stone-300 leading-relaxed mb-8 max-w-3xl">{stratText}</p>
            <Link href="/about/strategic-objective/" className="text-sm font-sans font-semibold text-iic-saffron underline-anim">
              Read more →
            </Link>
          </Reveal>
        </div>
        ...
        <div className="max-w-5xl mx-auto px-6 py-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-10 items-start mt-6">
            <Reveal className="md:col-span-3">
              <ImagePlaceholder
                alt="Professor Khinvraj Jangid — editorial portrait, square crop, neutral background, professional attire, eye contact"
                aspectRatio="1/1"
          ...
          <Reveal>
            <div className="grid grid-cols-1 md:grid-cols-12 gap-10 items-center">
              <div className="md:col-span-2">
                <ImagePlaceholder
                  alt="O.P. Jindal Global University crest — monochrome, transparent PNG, square format"
                  aspectRatio="1/1"
                  className="rounded max-w-[100px] md:max-w-full"
                />
              </div>
              <div className="md:col-span-10">
                ...
                <p className="text-lg text-stone-700 leading-relaxed mb-6">{partnersIntro}</p>
                <Link
