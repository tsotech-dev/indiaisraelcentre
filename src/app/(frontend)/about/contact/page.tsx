...
export const metadata: Metadata = {
  ...
  description:
    "Contact the India Israel Centre — editorial inquiries, media inquiries, institutional partnerships, and fellowship routes.",
};

const ROUTE_META = [
  { href: "/about/contact/editorial/", email: "", accent: "bg-iic-navy" },
  { href: "/about/contact/media/", email: "", accent: "bg-iic-saffron" },
...
const DEFAULT_ROUTES = [
  {
    ...
    description:
      "Proposals for publications, questions about research, corrections, and general editorial correspondence.",
  },
  {
    title: "Media inquiries",
    description:
      "Press contacts, background briefings, and requests for comment from journalists and media organisations.",
  },
  {
    title: "Partner with us",
    description:
      "Institutional partnership inquiries from research centres, universities, and foundations.",
  },
];

export default async function ContactPage() {
  const c = await getGlobal("about-contact");
  const routes = (c?.routes as { title: string; description: string }[] | null)
    ?.length
    ...
    <>
      ...
      <section className="bg-iic-paper border-b border-stone-200">
        <div className="max-w-7xl mx-auto px-6 py-16">
              ...
              return (
                <Reveal key={r.title} delay={Math.min(i * 80, 240)}>
                  <div className="group relative bg-white p-7 h-full hover:bg-iic-paper transition-colors">
                    <div
                      className={`absolute top-0 left-0 w-full h-1 ${meta?.accent ?? "bg-iic-navy"} scale-x-0 group-hover:scale-x-100 origin-left transition-transform duration-500`}
                    />
          ...
          <p className="text-sm text-stone-600 mt-10">
            For editorial and media enquiries, email @{" "}
            <Link
              href="/about/work-with-us/"
              className="text-iic-navy font-semibold underline-anim"
            >
