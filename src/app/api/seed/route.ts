...
export async function GET() {
  ...
  if (existingPaper.totalDocs === 0) {
    await payload.create({
      ...
      data: {
        ...
        abstract:
          "This paper examines the evolution of the India-Israel strategic partnership in the twenty-five years since diplomatic normalisation in 1992. We argue that the relationship has moved through three distinct phases — a convergence of interests phase, a formalisation phase, and an emerging strategic autonomy phase — characterised by a shift from platform procurement to co-development. Drawing on interviews with serving and retired defence officials and a systematic analysis of defence trade data, we assess the structural conditions that have enabled this shift and the constraints that remain.",
        body: richTextMulti([
          ...
          "The third phase, which we date from 2014 and characterise as ongoing, is defined by a shift from platform procurement to co-development. This shift was enabled by India's Make in India initiative, Israel's own evolution toward joint ventures and technology transfer, and a growing recognition that the relationship's long-term strategic value depends on deepening technological integration.",
        ]),
      },
    })
    results.push('Created paper: India-Israel Strategic Partnership')
  } else {
    results.push('Skipped paper (already exists)')
  }
  ...
  if (existingBrief.totalDocs === 0) {
    await payload.create({
      ...
      data: {
        ...
        type: 'brief',
        pillar: 'development',
        authors: [{ name: 'Ananya Krishnan', affiliation: 'India Israel Centre' }],
        ...
        body: richTextMulti([
          ...
          "This brief draws on fieldwork conducted in Punjab, Haryana, and Maharashtra, and on interviews with officials at the National Mission for Sustainable Agriculture and the Israel Innovation Authority. We find that the partnership's reach is substantial but geographically uneven, and that its future growth is constrained by three structural factors: limited absorptive capacity of state agricultural extension systems; intellectual property arrangements that create disincentives for technology adaptation; and a tendency to treat the partnership as a procurement relationship rather than a co-development one.",
    ...
    results.push('Created brief: Water Technology Cooperation')
  } else {
    results.push('Skipped brief (already exists)')
  }
  ...
  if (existingCommentary.totalDocs === 0) {
    ...
    results.push('Created commentary: The Abraham Accords and India\'s Strategic Calculus')
  } else {
    results.push('Skipped commentary (already exists)')
  }
  ...
  if (existingConvening.totalDocs === 0) {
    await payload.create({
      ...
      data: {
        title:
          'Roundtable on India-Israel Technology Cooperation: Defence, Agriculture, and Water',
        ...
        description: richTextMulti([
          ...
          "The roundtable was conducted under modified Chatham House rules: participants may be identified by category but not by name or precise institutional affiliation. This summary reflects the analytical ground covered; it does not attribute positions to individuals.",
          "The discussion identified a central tension in the current architecture of technology cooperation: the most significant opportunities — co-development of dual-use technologies, joint ventures in frontier agriculture — require institutional frameworks that neither side has yet constructed, while the existing framework is well-suited to older modalities that both sides have an interest in moving beyond.",
    ...
    results.push('Created convening: Technology Roundtable 2026')
  } else {
    results.push('Skipped convening (already exists)')
  }
