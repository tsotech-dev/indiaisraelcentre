...
export default async function HomePage() {
  const [home, latestPubs, upcomingConvenings, commentary] = await Promise.all([
    getGlobal("home"),
  ...
  // Featured publication: CMS pick → fallback to most recent
  const featuredId = home?.featuredPublication
  let featured = latestPubs[0] ?? null
  if (featuredId) {
    const slug = typeof featuredId === "object" ? (featuredId as { slug: string }).slug : null
    if (slug) {
      const found = latestPubs.find((p) => p.slug === slug)
      if (found) featured = found
    }
  }
  const rest = latestPubs.filter((p) => p.id !== featured?.id).slice(0, 3)

  const missionCards = (home?.missionCards as { title: string; body: string }[] | null)?.length
    ? (home?.missionCards as { title: string; body: string }[]).map((c, i) => ({
        ...DEFAULT_MISSION_CARDS[i],
        title: c.title,
        body: c.body,
      }))
    : DEFAULT_MISSION_CARDS

  const partners = (home?.partners as { name: string }[] | null)?.length
    ? (home?.partners as { name: string }[]).map((p) => p.name)
    : DEFAULT_PARTNERS

  const marqueeItems = (home?.marqueeItems as { text: string }[] | null)?.length
    ? (home?.marqueeItems as { text: string }[]).map((m) => m.text)
    : DEFAULT_MARQUEE
      ...
      <main className="overflow-hidden">
        ...
        <section className="relative bg-mesh bg-grid">
          ...
          <div
            ...
            style={{ background: "radial-gradient(circle, #005EB8 0%, transparent 65%)", animationDelay: "2s" }}
          />
          <div
            aria-hidden
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-end">
              <div className="lg:col-span-8">
                ...
                <p className="text-lg md:text-xl text-stone-600 leading-relaxed mb-10 max-w-2xl animate-fade-up delay-3">
                  ...
                  commentary — and bringing scholars, diplomats, and practitioners into the same room.
                </p>
          ...
          <div className="flex whitespace-nowrap animate-marquee">
            {[0, 1].map((dup) => (
              <div key={dup} className="flex items-center gap-12 px-6 shrink-0">
                {marqueeItems.map((t) => (
                  <span key={t} className="flex items-center gap-12 text-sm font-sans font-medium uppercase tracking-[0.2em] text-white/80">
                    <span>{t}</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-16">
              <div className="lg:col-span-5">
                ...
                <h2 className="font-display text-4xl md:text-5xl font-bold leading-tight text-stone-900 mb-6">
                  {home?.missionHeading ?? "A think tank built for the long argument."}
                </h2>
              </div>
              <div className="lg:col-span-7 lg:pt-4">
                <p className="text-lg text-stone-600 leading-relaxed">
                  {home?.missionBody ??
                    "We exist because the India–Israel relationship is consequential enough to deserve serious intellectual attention. The Centre produces research that meets academic standards while remaining usable to policy professionals, and convenes the people who actually move the relationship."}
                </p>
              </div>
            </div>
                  ...
                  >
                    <span
                      className={`absolute inset-x-0 top-0 h-1 ${
                        c.accent === "saffron" ? "bg-iic-saffron" : c.accent === "navy" ? "bg-iic-navy" : "bg-iic-gold"
                      }`}
                    />
                    ...
                    <h3 className="font-display text-2xl font-semibold text-stone-900 mb-3">{c.title}</h3>
            ...
            <div className="max-w-7xl mx-auto px-6 py-20 md:py-28">
              <Reveal>
                <div className="flex items-end justify-between mb-10">
                  ...
                  <Link href="/research/" className="hidden md:inline-flex items-center gap-1 text-sm font-sans font-semibold text-iic-navy underline-anim">
                    All publications →
                  </Link>
                </div>
              </Reveal>
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-5">
                    ...
                    <div className="md:col-span-3 p-10 bg-white">
                      ...
                      {featured.abstract && (
                        <p className="text-stone-600 leading-relaxed mb-6 line-clamp-3">{featured.abstract}</p>
                      )}
            ...
            <div className="max-w-7xl mx-auto px-6 py-20">
              <Reveal>
                <div className="flex items-end justify-between mb-10">
                  ...
                  <Link href="/research/" className="text-sm font-sans font-semibold text-iic-navy underline-anim">
                    View all →
                  </Link>
                </div>
              </Reveal>
            ...
            <Reveal>
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-12 items-end">
                <div className="lg:col-span-7">
                  <div className="text-[11px] font-sans font-bold uppercase tracking-[0.2em] text-iic-saffron mb-3">/ Six pillars</div>
                  <h2 className="font-display text-4xl md:text-5xl font-bold leading-tight text-stone-900">
                    {home?.pillarsHeading ?? "The full breadth of the relationship, organised."}
                  </h2>
                </div>
                <div className="lg:col-span-5">
                  <p className="text-stone-600 leading-relaxed">
                    {home?.pillarsBody ??
                      "Each pillar is a coherent research programme — not a tag. Together they map the relationship across identity, governance, security, technology, development, and culture."}
                  </p>
                </div>
              </div>
            </Reveal>
        ...
        <section className="relative bg-iic-paper border-y border-stone-200 overflow-hidden">
          ...
          <div className="relative max-w-5xl mx-auto px-6 py-24 md:py-32 text-center">
            <Reveal>
              <div className="font-display text-iic-saffron text-7xl md:text-9xl leading-none mb-4 opacity-40">"</div>
              <blockquote className="font-display text-3xl md:text-5xl font-light leading-tight text-stone-900 max-w-4xl mx-auto">
                {home?.pullQuote ??
                  'We bring people together around questions, not agendas. The output is the quality of understanding participants carry away — and occasionally, a written record of the ground that was covered.'}
              </blockquote>
              <div className="mt-10 inline-flex items-center gap-4">
                ...
                <div className="text-left">
                  <div className="font-sans font-semibold text-stone-900 text-sm">
                    {home?.pullQuoteAttribution ?? "Professor Khinvraj Jangid"}
                  </div>
                  <div className="text-xs font-sans text-stone-500 uppercase tracking-wider">
                    {home?.pullQuoteRole ?? "Chair, IIC Forum"}
                  </div>
                </div>
              </div>
            </Reveal>
          ...
          <div className="relative max-w-7xl mx-auto px-6 py-20 md:py-28">
            <Reveal>
              <div className="flex items-end justify-between mb-10 flex-wrap gap-4">
                <div>
                  <div className="text-[11px] font-sans font-bold uppercase tracking-[0.2em] text-iic-gold mb-3">/ The Forum</div>
                  <h2 className="font-display text-4xl md:text-5xl font-bold leading-tight">Upcoming convenings</h2>
                </div>
                <Link
                  href="/forum/"
                ...
                >
                  Full programme
                  <span className="bg-iic-gold text-iic-ink rounded-full w-8 h-8 flex items-center justify-center transition-transform duration-300 group-hover:translate-x-0.5">→</span>
                </Link>
              </div>
            </Reveal>
                  ...
                  <Reveal key={c.id} delay={i * 120}>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 mb-4 flex-wrap">
                        <span className="text-[10px] font-sans font-bold uppercase tracking-[0.18em] text-iic-gold bg-iic-gold/10 px-2.5 py-1 rounded-full">{c.format}</span>
                        {c.pillar && <span className="text-[10px] font-sans text-white/50 uppercase tracking-wider">{pillarLabel(c.pillar)}</span>}
                      </div>
                      <h3 className="font-display text-2xl font-semibold leading-snug mb-6 group-hover:text-iic-gold transition-colors">{c.title}</h3>
                      <div className="flex items-center justify-between pt-5 border-t border-white/10">
                        <time className="text-sm font-sans text-white/70 uppercase tracking-wider">{formatDate(c.eventDate)}</time>
                        <span className="text-iic-gold opacity-0 group-hover:opacity-100 transition-opacity">→</span>
                      </div>
                    </Link>
                  </Reveal>
                ))}
        ...
        <section className="bg-white">
          <div className="max-w-7xl mx-auto px-6 py-20 md:py-28">
            <Reveal>
              <div className="flex items-end justify-between mb-10 flex-wrap gap-4">
                <div>
                  <div className="text-[11px] font-sans font-bold uppercase tracking-[0.2em] text-iic-saffron mb-3">/ Commentary</div>
                  <h2 className="font-display text-4xl md:text-5xl font-bold text-stone-900">Timely analysis</h2>
                </div>
                <Link href="/research/commentary/" className="text-sm font-sans font-semibold text-iic-navy underline-anim">All commentary →</Link>
              </div>
            </Reveal>
            {commentary.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <section className="bg-iic-paper border-y border-stone-200 py-12 overflow-hidden">
          <div className="text-center mb-8">
            <div className="text-[11px] font-sans font-bold uppercase tracking-[0.2em] text-stone-400">In conversation with</div>
          </div>
          <div className="relative">
            ...
            <div className="flex whitespace-nowrap animate-marquee">
              {[0, 1].map((dup) => (
                <div key={dup} className="flex items-center gap-16 px-8 shrink-0">
                  {partners.map((p) => (
                    <span key={`${dup}-${p}`} className="font-display text-xl md:text-2xl font-semibold text-stone-400 shrink-0">{p}</span>
                  ))}
                </div>
              ))}
            </div>
          ...
          <div className="bg-mesh">
            <div className="max-w-4xl mx-auto px-6 py-24 md:py-32 text-center relative">
              <Reveal>
                ...
                <div className="text-[11px] font-sans font-bold uppercase tracking-[0.2em] text-iic-saffron mb-4">The Brief — Monthly</div>
                <h2 className="font-display text-4xl md:text-6xl font-bold leading-[1.05] text-stone-900 mb-5">
                  {home?.newsletterHeading ?? "Get the Centre's work, in your inbox."}
                </h2>
                <p className="text-lg text-stone-600 leading-relaxed mb-10 max-w-xl mx-auto">
                  {home?.newsletterDescription ??
                    "One monthly note from the editors: new publications, upcoming convenings, and a brief read from the chair. No filler. Unsubscribe any time."}
                </p>
                <div className="max-w-md mx-auto">
                  <NewsletterSignup source="homepage" variant="light" />
                </div>
                <p className="text-xs font-sans text-stone-400 mt-5">
                  {home?.newsletterPrivacyNote ?? "We confirm subscriptions by email. Your address is never shared."}
                </p>
              </Reveal>
            </div>
          </div>
